    ...
    for i in range(0, len(class_mean_centered)):
        for j in range(i+1, len(class_mean_centered)):
            ...
            cosine.append(
                ...
                / np.linalg.norm(class_mean_centered[i])
                / np.linalg.norm(class_mean_centered[j]))
            relative_norm.append(
                np.linalg.norm(class_mean_centered[i]-class_mean_centered[j])
                )
...
def get_same_encoding_fraction(evaluations):
    """
    Calculate the fraction of samples belonging to the same calss\
          that share the same binary encoding.

    Args:
        evaluations (dict): A dictionary containing the evaluation results.
            - 'x_penultimate' (numpy.ndarray): The penultimate layer activations of the model.
            - 'y_predicted' (numpy.ndarray): The predicted class labels.
            - 'y_label' (numpy.ndarray): The true class labels.

    Returns:
        float: The fraction of samples that have the same encoding as their predicted class label.
    """
